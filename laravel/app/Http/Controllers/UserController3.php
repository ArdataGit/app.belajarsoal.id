...

{
    ...
    public function paketsayadtl(Request $request, $idpaketdtl)
    {
        ...
        foreach($hasilujian as $h){
            if($h->bobot){
                $bobots--;
            }
        }
        foreach($hasilujiankecermatan as $h){
            if($h->bobot){
                $bobots--;
            }
        }
        ...
        if($bobothasil){
            ...
            foreach($hasilujian as $h){
                if($h->bobot){
                    ...
                    $totalkkm+= $h->kkm;
                    $totalnilai+= $h->nilai * (float)( $h->bobot / 100.0 );
            ...
            if($countkkm>0){
                ...
                $totalkkm = $totalkkm / $countkkm;

                $bobotlulus = $totalnilai;
        ...
        $data_param = [
            ...
            'detail_kats', 'cermat_kats', 'materi_kats', 'zoom_kats',
            'usebobot', 'bobothasil', 'bobotlulus', 'totalnilai', 'tab'
        ];
    ...
    public function mulaiujian(Request $request, $id)
    {
        ...
        $bobot = Crypt::decrypt($request->bobot[0]);
    ...
    public function detailhasil($id)
    {
        ...
        $upaketsoalmst = UMapelMst::select('id', 'fk_paket_dtl', 'fk_paket_mst' , 'jenis_waktu', 'jenis_pembahasan', 'pembahasan', 'judul', 'point', 'kkm', 'is_kkm', 'bagi_jawaban', 'jenis_penilaian')->find($id);
        ...
        if ($upaketsoalmst->is_mengerjakan == 1) {
            ...
            $paketdtl = PaketDtl::where('jenis',1)->where('fk_paket_mst', $upaketsoalmst->fk_paket_mst)->get();
            $paketdtlkecermatan = PaketDtl::where('jenis',2)->where('fk_paket_mst', $upaketsoalmst->fk_paket_mst)->get();

            $hasilujian = UMapelMst::where('fk_user_id', Auth::user()->id)->where('fk_paket_mst', $upaketsoalmst->fk_paket_mst)->where('is_mengerjakan', 0)->groupBy('fk_paket_dtl')->orderBy('created_at', 'desc')->get();
            $hasilujiankecermatan = UPaketSoalKecermatanMst::where('fk_user_id', Auth::user()->id)->where('fk_paket_mst', $upaketsoalmst->fk_paket_mst)->where('is_mengerjakan', 2)->groupBy('fk_paket_dtl')->orderBy('created_at', 'desc')->get();            
            ...
            $data_param = [
                ...
                'submenu', 'menu', 'upaketsoalmst', 'extend', 'usebobot', 'bobothasil'
            ];
    ...
    public function detailhasilkecermatan($id)
    {
        ...
        if ($upaketsoalmst->is_mengerjakan == 1) {
            ...
            $data_param = [
                ...
                'submenu', 'menu', 'upaketsoalmst', 'soalmst', 'cekbenar', 'ceksalah', 'hitungsoaldtl', 'extend', 'usebobot', 'bobothasil'
            ];
